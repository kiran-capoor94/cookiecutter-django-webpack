...
def remove_webpack_files():
    shutil.rmtree(os.path.join("{{ cookiecutter.project_slug }}", "assets"))
    file_names = ["webpack.config.js"]
    for file_name in file_names:
        os.remove(file_name)
...
def main():
    ...
    if (
        "{{ cookiecutter.use_docker }}".lower() == "n" and "{{ cookiecutter.use_heroku }}".lower() == "n"
    ):
    ...
    if (
        "{{ cookiecutter.js_task_runner }}".lower() != "none" and "{{ cookiecutter.use_docker }}".lower() == "y"
    ):
        print(
            WARNING +
    ...
    if "{{ cookiecutter.use_webpack }}".lower() == "n":
